    ...
    ngOnInit() {
        this.service.getEmployees().subscribe(list => {
            let array = list.map(item => {
                ...
                return Object.assign({ $key: item.key, departmentName: departmentName }, item.payload.val());
            });
...
let DepartmentService = class DepartmentService {
    ...
    getDepartmentName($key) {
        if ($key == "0") {
            ...
            return lodash__WEBPACK_IMPORTED_MODULE_3__["find"](this.array, (obj) => { return obj.$key == $key; })['name'];
        }
    }
};
